...
import { Shield, ArrowLeft, Search, DollarSign, TrendingUp, AlertCircle, CheckCircle } from 'lucide-react';
...
export default function CheckEligibility() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (interestRate <= 0 || interestRate > 50) throw new Error('Interest rate must be between 0.1% and 50%');
      if (tenure <= 0 || tenure > 360) throw new Error('Tenure must be between 1 and 360 months');
      ...
      const suggestedRate = creditScore >= 8 ? interestRate : interestRate + (10 - creditScore) * 0.5;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Enter loan details to check customer eligibility and get instant approval status.
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <div className="space-y-6">
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                      <div>
                        ...
                        <p className="text-2xl font-bold">{result.credit_score}/10</p>
                        <p className="text-sm text-gray-500">{getCreditScoreDescription(result.credit_score)}</p>
                      </div>
                      <div className="text-right">
                        <div className={`w-16 h-16 rounded-full flex items-center justify-center text-white font-bold text-lg ${
                          result.credit_score >= 8 ? 'bg-green-500' :
                          result.credit_score >= 6 ? 'bg-yellow-500' :
                          result.credit_score >= 4 ? 'bg-orange-500' : 'bg-red-500'
                        }`}>
                          {result.credit_score}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant={
                        result.approval_status === 'approved' ? 'default' :
                        result.approval_status === 'partial' ? 'secondary' : 'destructive'
                      } className="text-sm px-3 py-1">
                        {result.approval_status === 'approved' && <CheckCircle className="h-4 w-4 mr-1" />}
                        {result.approval_status === 'partial' && <AlertCircle className="h-4 w-4 mr-1" />}
                        {result.approval_status.charAt(0).toUpperCase() + result.approval_status.slice(1)}
                      </Badge>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-gray-600 dark:text-gray-400">Suggested Interest Rate</span>
                      <span className="font-semibold">{result.suggested_interest_rate}%</span>
                    </div>
                    ...
                    {result.approval_status !== 'rejected' && (
                      <div className="flex flex-col gap-3 pt-4">
                        ...
                        <Button asChild variant="outline" className="w-full">
                          <Link to="/check-eligibility">Check Another Customer</Link>
                        </Button>
                      </div>
                    )}
                  </CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">Credit Scoring System</CardTitle>
                </CardHeader>
