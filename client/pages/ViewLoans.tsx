...
export default function ViewLoans() {
  ...
  const generateMockLoans = (): Loan[] => {
    ...
    const customerNames = [
      ...
      'Emma Davis', 'James Miller', 'Lisa Anderson', 'Robert Taylor', 'Maria Garcia'
    ];
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      const monthlyPayment = Math.round((loanAmount * monthlyRate * Math.pow(1 + monthlyRate, tenure)) / 
                                       (Math.pow(1 + monthlyRate, tenure) - 1));
      
      const monthsElapsed = Math.floor((new Date().getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24 * 30));
    ...
    return mockLoans.sort((a, b) => new Date(b.start_date).getTime() - new Date(a.start_date).getTime());
  };
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {formatCurrency(loans.reduce((sum, loan) => sum + loan.loan_amount, 0))}
                  </p>
                </div>
                      ...
                      <TableRow key={loan.loan_id}>
                        ...
                        <TableCell>
                          <div>
                            ...
                            <p className="text-sm text-gray-500">ID: {loan.customer_id}</p>
                          </div>
                        </TableCell>
                        ...
                        <TableCell>
                          <div>
                            ...
                            <p className="text-sm text-gray-500">{loan.interest_rate}% APR</p>
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <Dialog>
                            ...
                            <DialogContent className="max-w-2xl">
                              <DialogHeader>
                                <DialogTitle>Loan Details - #{loan.loan_id}</DialogTitle>
                                <DialogDescription>
                                  ...
                                  <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-4">
                                      ...
                                      <div>
                                        ...
                                        <p className="font-semibold">{formatCurrency(selectedLoan.loan_amount)}</p>
                                      </div>
                                      <div>
                                        <p className="text-sm text-gray-600">Monthly EMI</p>
                                        <p className="font-semibold text-primary">{formatCurrency(selectedLoan.monthly_payment)}</p>
                                      </div>
                                      <div>
                                    ...
                                    <div className="space-y-4">
                                      <div>
                                        ...
                                        <p className="font-semibold">{formatDate(selectedLoan.start_date)}</p>
                                      </div>
                                      <div>
                                        <p className="text-sm text-gray-600">End Date</p>
                                        <p className="font-semibold">{formatDate(selectedLoan.end_date)}</p>
                                      </div>
                                      <div>
                                        <p className="text-sm text-gray-600">EMIs Paid On Time</p>
                                        <p className="font-semibold">{selectedLoan.emis_paid_on_time} / {selectedLoan.tenure - selectedLoan.remaining_months}</p>
                                      </div>
                                      <div>
                                  ...
                                  <div className="border-t pt-4">
                                    ...
                                    <div className="bg-gray-100 dark:bg-gray-800 rounded-lg p-4">
                                      <div className="flex justify-between mb-2">
                                        <span>Progress: {calculateProgress(selectedLoan)}%</span>
                                        <span>{selectedLoan.remaining_months} months remaining</span>
                                      </div>
